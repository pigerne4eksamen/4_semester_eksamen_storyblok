    ...
    <header className="sticky top-0 z-50 bg-[#F5EDE0] px-6 shadow-md md:px-20">
      <div className="flex items-center justify-between py-3 md:py-1">
        ...
        <button type="button" onClick={() => setIsOpen(true)} aria-label="Aabn menu" className="flex flex-col gap-1 md:hidden">
          <span className="h-0.5 w-6 bg-[#282828]" />
          <span className="h-0.5 w-6 bg-[#282828]" />
          <span className="h-0.5 w-6 bg-[#282828]" />
        </button>

        {/* Desktopmenu */}
        <nav className="hidden items-center gap-20 text-[#282828] md:flex">
          {/* Dropdown med ydelsessider */}
          <div className="relative" onMouseEnter={() => setDropdownOpen(true)} onMouseLeave={() => setDropdownOpen(false)}>
            <button type="button" className="flex cursor-pointer items-center gap-1 hover:text-[#50543B]">
              Murerarbejde <IoIosArrowDown />
            </button>

            {dropdownOpen && (
              <div className="absolute left-0 top-full mt-0 flex min-w-44 flex-col bg-[#F5EDE0] shadow-md">
                {serviceLinks.map((link) => (
                  <a key={link.href} href={link.href} className="p-4 hover:bg-[#fbf6ee]">
                    {link.label}
                  </a>
                ))}
              </div>
          ...
          <a href="/om-os" className="hover:text-[#50543B]">
            Om os
          </a>
          <a href="/galleri" className="hover:text-[#50543B]">
            Galleri
          </a>
          <a href="/kontakt-os" className="hover:text-[#50543B]">
            Kontakt
          </a>
        </nav>
      </div>
      ...
      {/* Mobilmenu */}
      <nav className={`fixed right-0 top-0 z-40 flex h-full w-[80%] max-w-sm flex-col gap-6 bg-[#F5EDE0] p-8 transition-transform duration-300 md:hidden ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
