...
export default function Galleri({ blok }: { blok: any }) {
  ...
  const billeder = (blok.images || []).filter((item: GalleryImage) => item.image?.filename);
        ...
        <div className="relative overflow-hidden px-4 pt-0 md:px-0 md:pt-1">
          <div className="relative h-[520px] md:h-[620px]">
            <div className="absolute left-[3%] top-[54%] z-10 hidden h-[82%] w-[25%] translate-y-[-54%] overflow-hidden rounded-xl md:block">
              <img src={prevImage.image.filename} alt={prevImage.image.alt || prevImage.label || ""} className="h-full w-full object-cover" />
            </div>
            ...
            <button type="button" aria-label="Forrige billede" onClick={() => setActiveIndex(getIndex(currentIndex - 1, billeder.length))} className="absolute left-4 top-1/2 z-30 flex h-12 w-14 -translate-y-1/2 items-center justify-center rounded-md bg-[#C44931] text-white shadow-lg shadow-black/20 transition hover:bg-[#A93B27] md:left-[15.5%] md:h-14 md:w-16">
              <IoIosArrowBack className="text-3xl md:text-4xl" />
            </button>

            <button type="button" aria-label="Næste billede" onClick={() => setActiveIndex(getIndex(currentIndex + 1, billeder.length))} className="absolute right-4 top-1/2 z-30 flex h-12 w-14 -translate-y-1/2 items-center justify-center rounded-md bg-[#C44931] text-white shadow-lg shadow-black/20 transition hover:bg-[#A93B27] md:right-[15.5%] md:h-14 md:w-16">
